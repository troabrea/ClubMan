...
namespace ClubMan.WebApi.Infrastructure;

public class AzulRemotePost
    {
        ...
        public AzulRemotePost(IHttpContextAccessor httpContextAccessor)
        {
        ...
        public void Post()
        {
            ...
            if (NewInputForEachValue)
            {
                foreach (string key in Params.Keys)
                {
                    ...
                    if (values != null)
                    {
                        ...
                        {
                            if (1 == 1 || key == "CURRENCYCODE")
                            {
                                sb.Append(
                                $"<input name=\"{WebUtility.HtmlEncode(key)}\" type=\"hidden\" value=\"{value}\">");
                            }
                            else
                            {
                                sb.Append(
            ...
            else
            {
                ...
                {
                    if (1 == 1 || Params.Keys[i].ToUpper() == "CURRENCYCODE")
                    {
                        sb.Append(
                            $"<input name=\"{WebUtility.HtmlEncode(Params.Keys[i])}\" type=\"hidden\" value=\"{Params[Params.Keys[i]]}\">");
                    }
                    else
                    {
                        sb.Append(
